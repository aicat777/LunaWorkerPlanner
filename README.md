...
- 对代码库任务，规划师会先让一个子代理在项目根目录创建或刷新 `.luna-worker-context.md`，其中只保留与当前目标相关的目录结构、关键模块、入口链路、测试方式和项目约束。
- 后续子代理仍收到自包含的任务说明，但只携带项目根目录和上下文文件路径；它们会先读取该文件，再定向检查任务相关代码，不需要每次重新理解整个项目。
- 代码执行任务结束时，子代理会直接维护 `.luna-worker-context.md`，更新可复用事实并删除过时内容。规划师不会在父对话中复制或转发上下文正文。
- 上下文文件建议保持在 600–1200 个英文单词以内，默认不得超过 1500 个英文单词，不记录命令输出、逐次任务日志或完整文件树。
